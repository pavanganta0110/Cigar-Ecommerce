...
Integration status is computed independently of each integration's own settings-page display logic, using the same `IntegrationStatus` value object: `disabled` (not configured), `sandbox` (a development-only mock, or an unapproved sandbox-only credential set, is active), or `connected` with a separate production-approval flag, so "configured" and "approved for production" are never conflated. This page is scoped to what is registered today — age verification, shipping, and payments. A tax integration added later should add its own health description and blocked-order query rather than this page guessing at an interface it does not yet know.
...
Checkout payment runs on the official **GlobalPayments Gateway Provider for WooCommerce** plugin (`global-payments-woocommerce`), using its Unified Payments product line (`globalpayments_gpapi`) under a Global Payments merchant account approved for tobacco/cigar sales. This repository does not implement its own card-handling, tokenization, or transaction code; it configures and gates the plugin's own gateway.

The plugin registers several gateway product lines and wallet/BNPL sub-gateways at once on activation (Heartland, Genius, Transit, Click to Pay, Google Pay, Apple Pay, Affirm, Clearpay, Klarna, bank payment, PayPal). Only Unified Payments has an approved merchant account for this store. `GlobalPaymentsRuntime::filterAvailableGateways()` unconditionally removes every other variant from `woocommerce_available_payment_gateways`, and removes Unified Payments itself whenever it is not configured, so an unapproved gateway can never appear at checkout regardless of what a store admin enables in the plugin's own settings screen.

### Credentials, sandbox/live separation, and production gate

`GlobalPaymentsConfiguration` reads sandbox and live App Id/App Key pairs from separate environment variables (`COMPADRES_GLOBAL_PAYMENTS_SANDBOX_APP_ID`/`_SANDBOX_APP_KEY` and `_APP_ID`/`_APP_KEY`) plus a required merchant contact URL (`COMPADRES_GLOBAL_PAYMENTS_MERCHANT_CONTACT_URL`), matching the plugin's own field split between its Live Mode toggle and its two independent credential sets. Live credentials and Live Mode are usable only when the application environment is `production` **and** `COMPADRES_PAYMENT_PRODUCTION_APPROVED=true` has been explicitly set, mirroring the same production-approval gate already used for FedEx shipping and AgeChecker age verification. Outside production, only sandbox credentials are ever considered, regardless of what live values are present in the environment.

`GlobalPaymentsRuntime::syncSettings()` writes only the fields this integration owns (`sandbox_app_id`, `sandbox_app_key`, `app_id`, `app_key`, `is_production`, `merchant_contact_url`, and `payment_interface`, forced to `hpp`) into the plugin's own settings option (`woocommerce_globalpayments_gpapi_settings`) on `admin_init`, so environment configuration stays authoritative over what could otherwise be hand-edited in the plugin's settings screen. Every other field the plugin exposes (enabled, title, transaction region, authorize-vs-capture, 3-D Secure, AVS/CVV rules, card-saving) remains under the store admin's control and is never touched by this sync. `payment_interface` is pinned to the plugin's hosted-payment-page (`hpp`) mode rather than its embedded drop-in field, so card data is captured on Global Payments' own hosted page and this application's PCI scope stays at SAQ A rather than a level requiring direct handling of card data.
