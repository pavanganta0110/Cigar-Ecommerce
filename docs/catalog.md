...
| Sales-tax classification (`sales_tax_classification`) | Controlled identifier; required for taxable cigar products | Empty or `cigar`; any other value is rejected | Variations inherit the parent classification | `_compadres_sales_tax_classification` | Internal only; authenticated REST; mapped and validated |
